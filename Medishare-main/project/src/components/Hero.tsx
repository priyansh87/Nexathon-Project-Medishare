import React from 'react';
import { Scan, Share2, Activity } from 'lucide-react';

export default function Hero() {
  return (
    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-gray-900 sm:text-5xl md:text-6xl">
            <span className="block">Reduce Medicine Waste</span>
            <span className="block text-emerald-600">Save Lives</span>
          </h1>
          <p className="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
            Connect unused medicines with those in need. Our AI-powered platform makes medicine donation smart, safe, and efficient.
          </p>
          <div className="mt-5 max-w-md mx-auto sm:flex sm:justify-center md:mt-8">
            <div className="rounded-md shadow">
              <a
                href="#donate"
                className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-emerald-600 hover:bg-emerald-700 md:py-4 md:text-lg md:px-10"
              >
                Donate Medicine
              </a>
            </div>
            <div className="mt-3 rounded-md shadow sm:mt-0 sm:ml-3">
              <a
                href="#ngos"
                className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-emerald-600 bg-white hover:bg-gray-50 md:py-4 md:text-lg md:px-10"
              >
                Find NGOs
              </a>
            </div>
          </div>
        </div>

        <div className="mt-20 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="flex items-center justify-center h-12 w-12 rounded-md bg-emerald-500 text-white mx-auto">
              <Scan className="h-6 w-6" />
            </div>
            <h3 className="mt-4 text-center text-lg font-medium text-gray-900">Smart Scanning</h3>
            <p className="mt-2 text-center text-gray-500">
              Scan medicine packaging to instantly verify authenticity and details
            </p>
          </div>

          <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="flex items-center justify-center h-12 w-12 rounded-md bg-emerald-500 text-white mx-auto">
              <Activity className="h-6 w-6" />
            </div>
            <h3 className="mt-4 text-center text-lg font-medium text-gray-900">Health Insights</h3>
            <p className="mt-2 text-center text-gray-500">
              Get personalized health insights from your medical reports
            </p>
          </div>

          <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="flex items-center justify-center h-12 w-12 rounded-md bg-emerald-500 text-white mx-auto">
              <Share2 className="h-6 w-6" />
            </div>
            <h3 className="mt-4 text-center text-lg font-medium text-gray-900">Smart Matching</h3>
            <p className="mt-2 text-center text-gray-500">
              AI matches your donations with NGOs and hospitals in need
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}